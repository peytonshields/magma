...
def construct_callgraph(args, binaries):
    ...
    if fuzzer:
        ...
        tmp = next(args.binaries_directory.glob(f"**/{args.fuzzer_name.name}.0.0.*.bc"))
        binaries = [tmp]
...
def main():
    ...
    if args.fuzzer_name:
        tmp = list(args.binaries_directory.glob(f"**/{args.fuzzer_name}.0.0.*.bc"))
        ...
        args.fuzzer_name = tmp[0].parent / args.fuzzer_name
        if not args.fuzzer_name.exists() or args.fuzzer_name.is_dir():
            parser.error(f"Couldn't find {args.fuzzer_name}.")
